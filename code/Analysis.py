...
from LP_SOR_and_DO import basicLP, OP_LP
...
class Analysis:
    ...
    def draw_DOs(self, solution=None, shortform=False, show_compartments="length", pdf=True,use_naive=False):
        ...
        return(dot_object)
    def draw_SORs(self, solution=None, shortform=False, show_species=False, show_compartments="length", show_overproduction=False, pdf=True,use_naive=False):
        
        options={"shortform":shortform, "show_species":show_species, "show_compartments":show_compartments, "show_overproduction":show_overproduction, "pdf":pdf}
        if solution==None:
            if hasattr(self,"SORs"):
                solution=self.SORs
            else:
        ...
        if show_overproduction==True:
            if not hasattr(self,"overproduction"):
                self.overproduction=self.get_ops()
