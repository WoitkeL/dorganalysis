...
def change_for_DO(model,RN,Rbool):
    
    species_exist = {sname: model.addVar( vtype=gp.GRB.BINARY, name="species_exist_{}".format(sname)) for sname in RN.species}
    model.setObjective(gp.quicksum(species_exist.values()), gp.GRB.MAXIMIZE)
    spec_closure=generate_closure_for_species(RN)
    #spec_closure= create_closures(lOR_solve, species=setOfSpecies, Timer_in_sec=120)
...
#involved species are part of the DO
    ...
    return(model,pattern1,pattern2)




#def basicLP(RN,ERCs,DO=False,DO2=False):
def basicLP(analyze,DO=False,DO2=False):
...
#the exact stoichiometric parameters for reactions and the resulting sspecies quantity
    ...
    if DO2==True:
        change_for_DO(model,analyze.reaction_network,Rbool)
    #else:
     #   model.setObjective(gp.quicksum(Rbool.values()), gp.GRB.MAXIMIZE)
    speciesdict={}
